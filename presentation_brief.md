...
>
> **Figures already exist** on `fixes-and-algorithms` at `report/figures/fig1…fig19.png` (generated by `report/build_report.py` + `tasks/mixed_answers/plot_mixed.py`). Each chart slide below names its file. One exception: the paper's `fig20` (conditional rates) was never committed — slide 10 handles that with a table.
...
Files the slides use: `fig1_accuracy_by_condition`, `fig2_capability_ladder`, `fig3_flip_rate`, `fig5_flip_vs_capability`, `fig7_translation_sources`, `fig10_translation_ensemble`, `fig14_hebrew_translations`, `fig19_mixed_by_gold_lang`. Slide 10 has **no committed figure** (paper's `fig20` missing) → use the table; nearest is `fig15_away_from_gold`. Useful extras not slotted: `fig9_translation_agreement`, `fig12_nllb_scaling`, `fig16–18_mixed_*`, `fig6_degradation_heatmap`, `fig11_accuracy_by_translator`, `fig13_flip_signed`, `fig8_training_devacc`.
