"""Full fine-tune of XLM-R on the English CSQA train split → checkpoints/xlmr-csqa.

Reads the 80-10-10 split written by ``make_split.py``:
  - train.jsonl  (80%) — fine-tune all of xlm-roberta-base + the MC head
  - dev.jsonl    (10%) — per-epoch eval; best-accuracy epoch is kept
  - test.jsonl   (10%) — English in-domain sanity check after training

This is English-only training (the cross-lingual transfer protocol): the model
never sees translated choices. The en-x flip evaluation runs separately via
``scripts.run_eval --arm xlmr`` on the official validation variants.

Usage:
    python -m training.xlmr_csqa.train                       # full run
...
def main() -> None:
    ...
    print(f"train={len(train)} dev={len(dev)} test={len(test)} -> ckpt {args.ckpt}")

    xlmr.train(args.ckpt, train_items=train, eval_items=dev,
               epochs=args.epochs, batch_size=args.batch_size, lr=args.lr,
               init_from=args.init_from)

    # English in-domain sanity check on the held-out test split.
    preds = xlmr.predict(test, args.ckpt)
